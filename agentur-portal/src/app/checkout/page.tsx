  ...
  async function handleSubmit() {
    ...
    try {
      const res = await fetch("/api/checkout/manual", {
        ...
        body: JSON.stringify({ paket: paket.id, zahlungsmodell, zahlungsart, profil: form, preis }),
      });
      const data = await res.json();
      if (data.customerId) {
        router.push("/checkout/bestaetigung?typ=manuell");
      } else {
        setApiError(data.error || "Ein Fehler ist aufgetreten. Bitte kontaktiere uns direkt.");
      }
    } catch {
        ...
        <div>
          {step === 1 && (
            <div>
              ...
              <Button className="w-full" size="lg" onClick={() => setStep(2)}>Weiter zu deinen Daten →</Button>
            </div>
          )}
            ...
            <div>
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                ...
                <div className="sm:col-span-2">
                  <Input label="E-Mail" type="email" required value={form.email} onChange={(e) => updateForm("email", e.target.value)} />
                </div>
                <div className="sm:col-span-2">
                  <Input label="Telefon" type="tel" required value={form.telefon} onChange={(e) => updateForm("telefon", e.target.value)} />
                </div>
                <div className="sm:col-span-2">
                  <Input label="Straße & Hausnummer" required value={form.strasse} onChange={(e) => updateForm("strasse", e.target.value)} />
                </div>
                ...
                <div className="sm:col-span-2">
                  <Input label="Land" required value={form.land} onChange={(e) => updateForm("land", e.target.value)} />
                </div>
          ...
          {step === 3 && (
            <div>
              ...
              <div className="space-y-3 mb-6">
                ...
                <Checkbox checked={datenschutz} onChange={setDatenschutz}
                  label={<>Ich habe die <Link href="/datenschutz" className="underline" target="_blank">Datenschutzerklärung</Link> gelesen.</>} />
                <Checkbox checked={widerruf} onChange={setWiderruf}
                  label={<>Ich habe die <Link href="/widerruf" className="underline" target="_blank">Widerrufsbelehrung</Link> zur Kenntnis genommen.</>} />
                <Checkbox checked={zahlungsInfo} onChange={setZahlungsInfo}
                  label={`Ich bestätige die Zahlungsart "${ZAHLUNGSARTEN.find((a) => a.id === zahlungsart)?.label}" verstanden zu haben.`} />
              </div>
              <div className="flex gap-3">
                <Button variant="outline" onClick={() => setStep(2)}>← Zurück</Button>
                <Button className="flex-1" size="lg" loading={loading} disabled={!canPay} onClick={handleSubmit}>
        ...
        <div className="lg:sticky lg:top-6 h-fit">
          <div className="rounded-xl border border-border bg-card p-6">
            ...
            <div className="space-y-2 text-sm mb-4">
              ...
              {aufpreis > 0 && (
                <div className="flex justify-between text-amber-600"><span>Ratenaufpreis</span><span>+{aufpreis}%</span></div>
              )}
              <div className="border-t border-border pt-2 flex justify-between font-bold text-base">
                <span>Gesamt</span><span>{formatCurrency(preis)}</span>
              </div>
            </div>
